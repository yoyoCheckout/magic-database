...
public class DispatcherImpl implements Dispatcher {
	...
	public Response getResult(Request request) throws Exception {
		...
		case Constants.BEFORE_SET:
			...
			break;
		case Constants.SET:
			bitcask.setWithExpireTime(request.getKey(), request.getValue(), request.getVersion(),
		...
		case Constants.GOSSIP_SET:
			// 同步节点信息：设置新值
			synchronizer.set(request.getKey(), request.getValue(), request.getVersion(), request.getExpireTime());
			response = null;
			break;
		default:
