...
public class SynchronizerImpl implements Synchronizer, Runnable {
	...
	@Override
	public void set(String key, String value, long version, long expireTime) throws Exception {
		bitcask.setWithExpireTime(key, value, version, expireTime);
	}
	...
	private void setRequest(BitCaskValue bcv) {
		sendRequest(
				new Request(Constants.GOSSIP_SET, bcv.getKey(), bcv.getValue(), bcv.getVersion(), bcv.getExpireTime()));
	}
	...
	private void sendRequest(String method, String key, String value, long version) {
		Request request = new Request(method, key, value, version);
		sendRequest(request);
	}
