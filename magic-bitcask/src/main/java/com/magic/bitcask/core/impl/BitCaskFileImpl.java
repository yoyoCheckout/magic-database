...
public class BitCaskFileImpl implements BitCaskFile {
	...
	@Override
	public BitCaskKey write(String key, String value, long version, long expireTime) throws IOException {
		BitCaskRecord record = fileEntry(key, value, version, expireTime);
		...
		return new BitCaskKey(fileId, version, expireTime, entryPos, entrySize, record.getCrc());
	}
	...
	private BitCaskRecord fileEntry(String key, String value, long version, long expireTime)
		...
		BitCaskRecord result = new BitCaskRecord(crcValue, version, expireTime, (short) keyBytes.length,
				valueBytes.length, key, value, vec);
		return result;
	}
	...
	public void doForEachKey(KeyIterator iter) throws Exception {
		...
		do {
			...
			iter.each(keyString, version, expireTime, pos - entrySize, entrySize, crc32);
		} while (read > 0);
	}
