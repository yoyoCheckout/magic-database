	...
	private void scanKeyFiles(BitCaskImpl result, File[] files, final BitCaskKeydir keydir) throws Exception {
		...
		for (File f : files) {
			...
			KeyIterator ki = new KeyIterator() {
				...
				@Override
				public void each(String key, long version, long expireTime, long position, int size, int crc32)
						throws Exception {
					BitCaskKey bck = new BitCaskKey(Util.getFileId(f), version, expireTime, position, size, crc32);
					keydir.put(key, bck);
